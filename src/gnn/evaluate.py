...
set, extracts GATConv attention weights (which edges matter most?), and runs
single-supplier and multi-node disruption scenario comparisons.
...
from src.gnn.train import BASELINE_SAVE_PATH, GNN_SAVE_PATH
...
def _collect(
    ...
    """Return ``(y_pred, y_true, x_all)`` concatenated over all batches in loader."""
    model.eval()
    ...
    return torch.cat(preds), torch.cat(targets), torch.cat(feats)


def _per_type_metrics(
    y_pred: torch.Tensor, y_true: torch.Tensor, x_all: torch.Tensor
) -> Dict[str, Dict[str, float]]:
    """Return ``{mse, mae, r2}`` per node layer type, inferred via argmax of one-hot slice.
    ...
    return {
        name: _compute_metrics(y_pred[layer_idx == li], y_true[layer_idx == li])
...
def _eval_scenario(
    ...
    """
    x = torch.from_numpy(
        scaler.transform(np.array(run["initial_features"], dtype=np.float32)).astype(np.float32)
    )
    ...
    gp = gnn(data.x, data.edge_index, None).cpu()
    bp = baseline(data.x, data.edge_index, None).cpu()
    y_cpu = y.cpu()
    order: List[str] = run["node_order"]
    return {
...
def run_evaluation(
    ...
    gnn: DTNetGNN = DTNetGNN(in_channels=in_channels).to(device)
    gnn.load_state_dict(torch.load(GNN_SAVE_PATH, map_location=device))
    baseline: IsolatedBaseline = IsolatedBaseline(in_channels=in_channels).to(device)
    ...
    gnn_pred, y_true, x_all = _collect(gnn, test_loader, device)
    ...
    per_type_gnn: Dict[str, Dict] = _per_type_metrics(gnn_pred, y_true, x_all)
    per_type_base: Dict[str, Dict] = _per_type_metrics(base_pred, y_true, x_all)
    ...
    for name in LAYER_ORDER:
        ...
        if not gm:
            continue
        g_str = f"GNN  mse={gm['mse']:.4f} mae={gm['mae']:.4f} r2={gm['r2']:.3f}"
        b_str = (f"Base mse={bm['mse']:.4f} mae={bm['mae']:.4f} r2={bm['r2']:.3f}"
                 if bm else "")
    ...
    if runs is not None:
        ...
        for key, label in [("single_supplier", "Single Supplier Failure"),
            ...
            if rd is None:
                ...
                continue
            res: Dict[str, Any] = _eval_scenario(gnn, baseline, rd, scaler, device)
